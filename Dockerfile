...
RUN bench get-app --branch "${ERPNEXT_BRANCH}" "${ERPNEXT_REPO}" \
	&& cd apps/erpnext \
	&& git fetch --depth 1 origin "${ERPNEXT_REF}" \
	&& git checkout "${ERPNEXT_REF}"
...
COPY --chown=frappe:frappe scripts/railway-entrypoint.sh /home/frappe/frappe-bench/railway-entrypoint.sh
COPY --chown=frappe:frappe scripts/railway-bootstrap.sh /home/frappe/frappe-bench/railway-bootstrap.sh
COPY --chown=frappe:frappe scripts/railway-migrate.sh /home/frappe/frappe-bench/railway-migrate.sh
RUN chmod +x \
